...
fetch('Footer.html')
    .then(r => r.text())
    .then(data => document.getElementById('footer-placeholder').innerHTML)
...
function displaySummaries(summaries) {
    ...
    if (!summaries || summaries.length === 0) {
        container.innerHTML = '<div class="no-results">لا يوجد تلخيصات مطابقة</div>';
        return;
    }
    summaries.forEach(s => {
        ...
        card.innerHTML = `
            ...
            <div class="summary-info"><strong>التخصص:</strong> ${s.specializationName || 'عام'}</div>
...
function filterSummaries() {
    ...
    if (selectedSpec) filtered = filtered.filter(s => s.specializationId == selectedSpec);
...
document.getElementById('specialization').addEventListener('change', filterSummaries);
